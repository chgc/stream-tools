...
namespace stream_tools
{
    public class ActionHub : Hub
    {
        ...
        public async Task SendNameList(string roomName, Models.GameInfo gameInfo)
        {
            var settings = new JsonSerializerSettings()
            {
                ContractResolver = new CamelCasePropertyNamesContractResolver()
            };
            await Clients.Group(roomName).SendAsync("ReceiveNameList", JsonConvert.SerializeObject(gameInfo, settings));
        }
    }
}
